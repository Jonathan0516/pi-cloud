...
import type { OpenSessionLease } from "./lease.ts";
import type { PostgresSessionMetadata } from "./session/session-row.ts";

export interface PostgresOpenSessionOptions {
	/** Runs after the underlying session closed; awaited before `close()` settles. */
	onClose: () => void | Promise<void>;
	/** The lease this repository holds for the session, when leases are enabled. */
	lease?: OpenSessionLease;
}

/** Repository-owned open-session wrapper: tracks admitted work so close can drain it, then releases the id. */
export class PostgresOpenSession implements Session<PostgresSessionMetadata> {
	readonly metadata: PostgresSessionMetadata;
	readonly idGenerator: Session<PostgresSessionMetadata>["idGenerator"];
	/** Lease held for this session, or undefined when the repository runs without leases. */
	readonly lease: OpenSessionLease | undefined;
	private readonly session: Session<PostgresSessionMetadata>;
	...
	constructor(session: Session<PostgresSessionMetadata>, options: PostgresOpenSessionOptions) {
